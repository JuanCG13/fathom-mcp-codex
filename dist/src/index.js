...
const client = (account) => createFathomClient({ account });
const triggeredForSchema = z
    ...
    .min(1);
server.registerTool("fathom_list_accounts", {
...
server.registerTool("fathom_get_default_account", {
...
server.registerTool("fathom_list_meetings", {
...
}, async (input) => jsonText(await client(input.account).get("/meetings", {
    cursor: input.cursor,
...
server.registerTool("fathom_get_recording_summary", {
...
server.registerTool("fathom_get_recording_transcript", {
...
server.registerTool("fathom_get_recording_content", {
...
server.registerTool("fathom_list_teams", {
...
server.registerTool("fathom_list_team_members", {
...
server.registerTool("fathom_create_webhook", {
    ...
    return jsonText(await client(input.account).post("/webhooks", {
...
server.registerTool("fathom_delete_webhook", {
...
server.registerTool("fathom_verify_webhook", {
    title: "Verify Fathom webhook signature",
    description: "Verify Fathom webhook headers and raw request body using FATHOM_WEBHOOK_SECRET or an explicit secret.",
    inputSchema: {
...
}, async ({ headers, raw_body, secret, tolerance_seconds }) => jsonText({
